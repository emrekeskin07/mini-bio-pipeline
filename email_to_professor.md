...
3. Visual Reporting: We generated a QC dashboard combining five plots: GC content distribution, read length distributions (linear and log scale), quality score histogram, and violin plot, alongside a summary statistics table.
...
- N50: 1,761 bp meaning that 50% of all sequenced bases come from reads longer than 1,761 bp
- The longest single read was 686,155 bp, ultra-long reads like this are a hallmark of Nanopore sequencing
- The distribution is right-skewed: most reads are short, but a tail of very long reads pulls the mean upward

Are the read lengths as expected? Yes. For Oxford Nanopore sequencing, a right-skewed distribution with a median around 500-1000 bp and occasional ultra-long reads is entirely normal. The N50 of 1,761 bp confirms that a substantial portion of the data consists of informative long reads. There are no signs of DNA degradation or library preparation issues.

### GC Content

- Mean GC: 53.0%, Median: 53.5%
- The distribution follows a clean, symmetric bell curve centred around 53%
- A skewed or bimodal GC distribution would suggest contamination, we see no such signal here
...
- Only 8.5% of reads exceed Q15, typical for R9.4.1 or earlier Nanopore chemistry
- The violin plot reveals a bimodal quality distribution, this is common and does not affect alignment significantly

Is the quality sufficient? Yes. The median quality of Q17.3 comfortably exceeds the Q10 threshold that is standard for Nanopore long-read data. With 77.7% of reads above Q10, the dataset is well-suited for downstream alignment and variant calling. The bimodal distribution is a known characteristic of Nanopore chemistry and does not indicate a problem.

---

## Recommendation

Based on these results, we recommend proceeding to alignment. The data meets all standard quality criteria:

- Read lengths are within expected Nanopore range
...
2. Align to reference genome using Minimap2 with -ax map-ont preset
